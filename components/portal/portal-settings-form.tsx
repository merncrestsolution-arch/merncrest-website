...
export function PortalSettingsForm() {
  ...
  return (
    <div className="space-y-4">
      {message && <p className="text-sm text-[#28a745]">{message}</p>}
      {error && <p className="rlk-login-error">{error}</p>}
      ...
      <form onSubmit={save} className="space-y-4">
          ...
          <div className="rlk-section-body space-y-3">
            <div className="grid sm:grid-cols-2 gap-3">
              <label className="text-xs text-[#666] block">
                Preferred language
                <select
                  className="rlk-input mt-1 w-full"
        ...
        <button type="submit" className="rlk-btn-green !w-auto" disabled={saving}>
      ...
      <section className="rlk-section rlk-section-accent-teal">
        ...
        {loginHistory.length === 0 ? (
          ...
          <ul className="space-y-2 text-sm">
            {loginHistory.map((l) => (
              <li key={l.id} className="flex flex-wrap justify-between gap-2 border-b border-[#e4dff0] py-2 dark:border-white/5">
...
function Field({
  ...
  return (
    <label className={`text-xs text-[#666] block ${className || ""}`}>
      {label}
      <input
        required={required}
        className="rlk-input mt-1 w-full"
